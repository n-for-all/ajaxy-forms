...
use Isolated\Doctrine\Common\Lexer\Token;
use Isolated\Egulias\EmailValidator\EmailLexer;
use Isolated\Egulias\EmailValidator\Warning\TLD;
...
use Isolated\Egulias\EmailValidator\Result\Reason\DotAtEnd;
use Isolated\Egulias\EmailValidator\Result\Reason\DotAtStart;
use Isolated\Egulias\EmailValidator\Warning\DeprecatedComment;
use Isolated\Egulias\EmailValidator\Result\Reason\CRLFAtTheEnd;
use Isolated\Egulias\EmailValidator\Result\Reason\LabelTooLong;
use Isolated\Egulias\EmailValidator\Result\Reason\NoDomainPart;
use Isolated\Egulias\EmailValidator\Result\Reason\ConsecutiveAt;
use Isolated\Egulias\EmailValidator\Result\Reason\DomainTooLong;
use Isolated\Egulias\EmailValidator\Result\Reason\CharNotAllowed;
use Isolated\Egulias\EmailValidator\Result\Reason\DomainHyphened;
use Isolated\Egulias\EmailValidator\Result\Reason\ExpectingATEXT;
use Isolated\Egulias\EmailValidator\Parser\CommentStrategy\DomainComment;
use Isolated\Egulias\EmailValidator\Result\Reason\ExpectingDomainLiteralClose;
use Isolated\Egulias\EmailValidator\Parser\DomainLiteral as DomainLiteralParser;
class DomainPart extends PartParser
{
    ...
    {
        $thereIsNoDomain = ((array) $this->lexer->token)['type'] === EmailLexer::S_EMPTY || ((array) $this->lexer->token)['type'] === EmailLexer::S_SP && !$this->lexer->isNextToken(EmailLexer::GENERIC);
        if ($thereIsNoDomain) {
    ...
    protected function doParseDomainPart() : Result
    {
        ...
        do {
            ...
            if (((array) $this->lexer->token)['type'] === EmailLexer::S_OPENPARENTHESIS || ((array) $this->lexer->token)['type'] === EmailLexer::S_CLOSEPARENTHESIS) {
                $hasComments = \true;
    ...
    protected function checkDomainPartExceptions(array $prev, bool $hasComments) : Result
    {
        ...
        if (((array) $this->lexer->token)['type'] === EmailLexer::S_BACKSLASH && $this->lexer->isNextToken(EmailLexer::GENERIC)) {
