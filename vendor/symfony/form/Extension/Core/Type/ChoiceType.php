...
use Isolated\Symfony\Component\Form\ChoiceList\Factory\Cache\ChoiceAttr;
use Isolated\Symfony\Component\Form\ChoiceList\Factory\Cache\ChoiceFieldName;
use Isolated\Symfony\Component\Form\ChoiceList\Factory\Cache\ChoiceFilter;
use Isolated\Symfony\Component\Form\ChoiceList\Factory\Cache\ChoiceLabel;
use Isolated\Symfony\Component\Form\ChoiceList\Factory\Cache\ChoiceLoader;
use Isolated\Symfony\Component\Form\ChoiceList\Factory\Cache\ChoiceTranslationParameters;
use Isolated\Symfony\Component\Form\ChoiceList\Factory\Cache\ChoiceValue;
use Isolated\Symfony\Component\Form\ChoiceList\Factory\Cache\GroupBy;
use Isolated\Symfony\Component\Form\ChoiceList\Factory\Cache\PreferredChoice;
...
use Isolated\Symfony\Component\Form\Extension\Core\DataMapper\CheckboxListMapper;
use Isolated\Symfony\Component\Form\Extension\Core\DataMapper\RadioListMapper;
use Isolated\Symfony\Component\Form\Extension\Core\DataTransformer\ChoicesToValuesTransformer;
use Isolated\Symfony\Component\Form\Extension\Core\DataTransformer\ChoiceToValueTransformer;
use Isolated\Symfony\Component\Form\Extension\Core\EventListener\MergeCollectionListener;
...
class ChoiceType extends AbstractType
{
    ...
    {
        $this->choiceListFactory = $choiceListFactory ?? new CachingFactoryDecorator(new PropertyAccessDecorator(new DefaultChoiceListFactory()));
    ...
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        if ($options['multiple'] && $options['by_reference']) {
            ...
            $builder->addEventSubscriber(new MergeCollectionListener(\true, \true));
        }
    ...
    public function buildView(FormView $view, FormInterface $form, array $options)
    {
        ...
        $choiceListView = $form->getConfig()->hasAttribute('choice_list_view') ? $form->getConfig()->getAttribute('choice_list_view') : $this->createChoiceListView($choiceList, $options);
        $view->vars = \array_replace($view->vars, ['multiple' => $options['multiple'], 'expanded' => $options['expanded'], 'preferred_choices' => $choiceListView->preferredChoices, 'choices' => $choiceListView->choices, 'separator' => '-------------------', 'placeholder' => null, 'choice_translation_domain' => $choiceTranslationDomain, 'choice_translation_parameters' => $options['choice_translation_parameters']]);
        ...
        if ($options['multiple']) {
            $view->vars['is_selected'] = function ($choice, array $values) {
                return \in_array($choice, $values, \true);
            };
        } else {
    ...
    public function configureOptions(OptionsResolver $resolver)
    {
        ...
        $choiceTranslationDomainNormalizer = function (Options $options, $choiceTranslationDomain) {
            if (\true === $choiceTranslationDomain) {
        ...
        $resolver->setDefaults([
            ...
            'data_class' => null,
            'choice_translation_domain' => \true,
            'trim' => \false,
            'invalid_message' => function (Options $options, $previousValue) {
                return $options['legacy_error_messages'] ?? \true ? $previousValue : 'The selected choice is invalid.';
            },
        ]);
    ...
    {
        $choiceOpts = ['value' => $choiceView->value, 'label' => $choiceView->label, 'label_html' => $options['label_html'], 'attr' => $choiceView->attr, 'label_translation_parameters' => $choiceView->labelTranslationParameters, 'translation_domain' => $options['choice_translation_domain'], 'block_name' => 'entry'];
        if ($options['multiple']) {
    ...
    private function createChoiceList(array $options)
    {
        if (null !== $options['choice_loader']) {
            return $this->choiceListFactory->createListFromLoader($options['choice_loader'], $options['choice_value'], $options['choice_filter']);
        }
        // Harden against NULL values (like in EntityType and ModelType)
        $choices = null !== $options['choices'] ? $options['choices'] : [];
        return $this->choiceListFactory->createListFromChoices($choices, $options['choice_value'], $options['choice_filter']);
    }
    private function createChoiceListView(ChoiceListInterface $choiceList, array $options)
    {
        return $this->choiceListFactory->createView($choiceList, $options['preferred_choices'], $options['choice_label'], $options['choice_name'], $options['group_by'], $options['choice_attr'], $options['choice_translation_parameters']);
    }
}
