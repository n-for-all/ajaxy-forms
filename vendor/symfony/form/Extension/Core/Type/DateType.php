...
class DateType extends AbstractType
{
    ...
    private const WIDGETS = ['text' => TextType::class, 'choice' => ChoiceType::class];
    /**
     * {@inheritdoc}
     */
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        if ('single_text' === $options['widget']) {
            ...
            $yearOptions = $monthOptions = $dayOptions = ['error_bubbling' => \true, 'empty_data' => ''];
            ...
            $builder->add('year', self::WIDGETS[$options['widget']], $yearOptions)->add('month', self::WIDGETS[$options['widget']], $monthOptions)->add('day', self::WIDGETS[$options['widget']], $dayOptions)->addViewTransformer(new DateTimeToArrayTransformer($options['model_timezone'], $options['view_timezone'], ['year', 'month', 'day']))->setAttribute('formatter', $formatter);
        }
        if ('datetime_immutable' === $options['input']) {
            ...
            $builder->addModelTransformer(new ReversedTransformer(new DateTimeToArrayTransformer($options['model_timezone'], $options['model_timezone'], ['year', 'month', 'day'])));
        }
    }
    /**
     * {@inheritdoc}
    ...
    public function configureOptions(OptionsResolver $resolver)
    {
        ...
        $choiceTranslationDomainNormalizer = function (Options $options, $choiceTranslationDomain) {
            if (\is_array($choiceTranslationDomain)) {
                $default = \false;
                return \array_replace(['year' => $default, 'month' => $default, 'day' => $default], $choiceTranslationDomain);
            }
            return ['year' => $choiceTranslationDomain, 'month' => $choiceTranslationDomain, 'day' => $choiceTranslationDomain];
        };
        ...
        $resolver->setDefaults([
            ...
            'invalid_message' => function (Options $options, $previousValue) {
                return $options['legacy_error_messages'] ?? \true ? $previousValue : 'Please enter a valid date.';
            },
        ]);
        $resolver->setNormalizer('placeholder', $placeholderNormalizer);
        $resolver->setNormalizer('choice_translation_domain', $choiceTranslationDomainNormalizer);
        $resolver->setAllowedValues('input', ['datetime', 'datetime_immutable', 'string', 'timestamp', 'array']);
        $resolver->setAllowedValues('widget', ['single_text', 'text', 'choice']);
