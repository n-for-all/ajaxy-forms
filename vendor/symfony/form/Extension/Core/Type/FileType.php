...
class FileType extends AbstractType
{
    ...
    {
        $dataClass = null;
        if (\class_exists(\Isolated\Symfony\Component\HttpFoundation\File\File::class)) {
        ...
        $resolver->setDefaults(['compound' => \false, 'data_class' => $dataClass, 'empty_data' => $emptyData, 'multiple' => \false, 'allow_file_upload' => \true, 'invalid_message' => function (Options $options, $previousValue) {
            return $options['legacy_error_messages'] ?? \true ? $previousValue : 'Please select a valid file.';
        }]);
    }
    /**
     * {@inheritdoc}
    ...
    private function getFileUploadError(int $errorCode)
    {
        ...
        if (\UPLOAD_ERR_INI_SIZE === $errorCode) {
            ...
            $messageParameters = ['{{ limit }}' => $limitAsString, '{{ suffix }}' => $suffix];
