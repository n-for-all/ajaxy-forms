...
use Isolated\Symfony\Component\Form\Extension\Core\DataTransformer\WeekToArrayTransformer;
...
{
    private const WIDGETS = ['text' => IntegerType::class, 'choice' => ChoiceType::class];
    /**
     * {@inheritdoc}
     */
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        if ('single_text' === $options['widget']) {
            ...
            $yearOptions = $weekOptions = ['error_bubbling' => \true, 'empty_data' => ''];
    ...
    public function configureOptions(OptionsResolver $resolver)
    {
        ...
        $choiceTranslationDomainNormalizer = function (Options $options, $choiceTranslationDomain) {
            if (\is_array($choiceTranslationDomain)) {
                $default = \false;
                return \array_replace(['year' => $default, 'week' => $default], $choiceTranslationDomain);
            }
            return ['year' => $choiceTranslationDomain, 'week' => $choiceTranslationDomain];
        };
        $resolver->setDefaults(['years' => \range(\date('Y') - 10, \date('Y') + 10), 'weeks' => \array_combine(\range(1, 53), \range(1, 53)), 'widget' => 'single_text', 'input' => 'array', 'placeholder' => $placeholderDefault, 'html5' => static function (Options $options) {
            return 'single_text' === $options['widget'];
        }, 'error_bubbling' => \false, 'empty_data' => function (Options $options) {
            return $options['compound'] ? [] : '';
        }, 'compound' => $compound, 'choice_translation_domain' => \false, 'invalid_message' => static function (Options $options, $previousValue) {
            return $options['legacy_error_messages'] ?? \true ? $previousValue : 'Please enter a valid week.';
        }]);
        ...
        $resolver->setAllowedValues('input', ['string', 'array']);
