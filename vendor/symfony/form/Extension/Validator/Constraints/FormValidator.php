...
class FormValidator extends ConstraintValidator
{
    ...
    public function validate($form, Constraint $formConstraint)
    {
        ...
        if ($form->isSubmitted() && $form->isSynchronized()) {
            ...
            $validateDataGraph = $form->isRoot() && (\is_object($data) || \is_array($data)) && ($groups && \is_array($groups) || $groups instanceof GroupSequence && $groups->groups);
            ...
            if ($childrenSynchronized) {
                ...
                $this->context->buildViolation($failure->getInvalidMessage() ?? $config->getOption('invalid_message'))->setParameters(\array_replace(['{{ value }}' => $clientDataAsString], $config->getOption('invalid_message_parameters'), $failure->getInvalidMessageParameters()))->setInvalidValue($form->getViewData())->setCode(Form::NOT_SYNCHRONIZED_ERROR)->setCause($failure)->addViolation();
            }
        }
        // Mark the form with an error if it contains extra fields
        if (!$config->getOption('allow_extra_fields') && \count($form->getExtraData()) > 0) {
            $this->context->setConstraint($formConstraint);
            $this->context->buildViolation($config->getOption('extra_fields_message', ''))->setParameter('{{ extra_fields }}', '"' . \implode('", "', \array_keys($form->getExtraData())) . '"')->setPlural(\count($form->getExtraData()))->setInvalidValue($form->getExtraData())->setCode(Form::NO_SUCH_FIELD_ERROR)->addViolation();
        }
    }
    /**
    ...
    private static function getConstraintsInGroups($constraints, $group)
    {
        ...
        return \array_filter($constraints, static function (Constraint $constraint) use($groups) {
            foreach ($groups as $group) {
                if (\in_array($group, $constraint->groups, \true)) {
                    return \true;
                }
            }
            return \false;
