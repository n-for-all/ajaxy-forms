...
use Isolated\Symfony\Component\Form\FileUploadError;
...
use Isolated\Symfony\Component\PropertyAccess\PropertyPathBuilder;
use Isolated\Symfony\Component\PropertyAccess\PropertyPathIterator;
use Isolated\Symfony\Component\PropertyAccess\PropertyPathIteratorInterface;
    ...
    public function mapViolation(ConstraintViolation $violation, FormInterface $form, bool $allowNonSynchronized = \false)
    {
        ...
        if ($this->acceptsErrors($scope)) {
            ...
            if (\false !== \strpos($message, '{{ label }}') || \false !== \strpos($messageTemplate, '{{ label }}')) {
                ...
                if (null !== $labelFormat) {
                    $label = \str_replace(['%name%', '%id%'], [$scope->getName(), (string) $scope->getPropertyPath()], $labelFormat);
                } else {
                ...
                if (\false !== $label) {
                    ...
                    if (null !== $this->translator) {
                        ...
                        do {
                            ...
                            \array_unshift($translationParameters, $form->getConfig()->getOption('label_translation_parameters', []));
            ...
            $scope->addError(new FormError($message, $messageTemplate, $violation->getParameters(), $violation->getPlural(), $violation));
        }
    }
    /**
