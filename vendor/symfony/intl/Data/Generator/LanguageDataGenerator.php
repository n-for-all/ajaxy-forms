...
use Isolated\Symfony\Component\Intl\Data\Util\ArrayAccessibleResourceBundle;
...
class LanguageDataGenerator extends AbstractDataGenerator
{
    ...
    protected function generateDataForLocale(BundleEntryReaderInterface $reader, string $tempDir, string $displayLocale) : ?array
    {
        ...
        if (isset($localeBundle['Languages']) && null !== $localeBundle['Languages']) {
            ...
            $data = ['Names' => $names, 'LocalizedNames' => $localizedNames];
            return $data;
        }
        return null;
    }
    ...
    protected function generateDataForMeta(BundleEntryReaderInterface $reader, string $tempDir) : ?array
    {
        ...
        return ['Languages' => $this->languageCodes, 'Alpha3Languages' => $this->generateAlpha3Codes($this->languageCodes, $metadataBundle), 'Alpha2ToAlpha3' => $this->generateAlpha2ToAlpha3Mapping($metadataBundle), 'Alpha3ToAlpha2' => $this->generateAlpha3ToAlpha2Mapping($metadataBundle)];
    }
    ...
    private function generateAlpha3ToAlpha2Mapping(ArrayAccessibleResourceBundle $metadataBundle) : array
    {
        ...
        foreach ($metadataBundle['alias']['language'] as $alias => $data) {
            $language = $data['replacement'];
            if (2 === \strlen($language) && 3 === \strlen($alias) && \in_array($data['reason'], ['overlong', 'bibliographic'], \true)) {
