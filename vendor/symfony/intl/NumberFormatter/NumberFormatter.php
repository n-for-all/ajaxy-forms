 ...
 */
namespace Isolated\Symfony\Component\Intl\NumberFormatter;

use Isolated\Symfony\Component\Intl\Currencies;
use Isolated\Symfony\Component\Intl\Exception\MethodArgumentNotImplementedException;
...
use Isolated\Symfony\Component\Intl\Exception\NotImplementedException;
...
abstract class NumberFormatter
{
     ...
     */
    private const SUPPORTED_STYLES = ['CURRENCY' => self::CURRENCY, 'DECIMAL' => self::DECIMAL];
    /**
     * Supported attributes to the setAttribute() $attr argument.
     */
    private const SUPPORTED_ATTRIBUTES = ['FRACTION_DIGITS' => self::FRACTION_DIGITS, 'GROUPING_USED' => self::GROUPING_USED, 'ROUNDING_MODE' => self::ROUNDING_MODE];
    /**
     ...
     */
    private const ROUNDING_MODES = ['ROUND_HALFEVEN' => self::ROUND_HALFEVEN, 'ROUND_HALFDOWN' => self::ROUND_HALFDOWN, 'ROUND_HALFUP' => self::ROUND_HALFUP, 'ROUND_CEILING' => self::ROUND_CEILING, 'ROUND_FLOOR' => self::ROUND_FLOOR, 'ROUND_DOWN' => self::ROUND_DOWN, 'ROUND_UP' => self::ROUND_UP];
    /**
     ...
     */
    private const PHP_ROUNDING_MAP = [self::ROUND_HALFDOWN => \PHP_ROUND_HALF_DOWN, self::ROUND_HALFEVEN => \PHP_ROUND_HALF_EVEN, self::ROUND_HALFUP => \PHP_ROUND_HALF_UP];
    /**
     ...
     */
    private const CUSTOM_ROUNDING_LIST = [self::ROUND_CEILING => \true, self::ROUND_FLOOR => \true, self::ROUND_DOWN => \true, self::ROUND_UP => \true];
    /**
    ...
    private const EN_SYMBOLS = [self::DECIMAL => ['.', ',', ';', '%', '0', '#', '-', '+', '¤', '¤¤', '.', 'E', '‰', '*', '∞', 'NaN', '@', ','], self::CURRENCY => ['.', ',', ';', '%', '0', '#', '-', '+', '¤', '¤¤', '.', 'E', '‰', '*', '∞', 'NaN', '@', ',']];
    private const EN_TEXT_ATTRIBUTES = [self::DECIMAL => ['', '', '-', '', ' ', 'XXX', ''], self::CURRENCY => ['¤', '', '-¤', '', ' ', 'XXX']];
    /**
    ...
    public function formatCurrency(float $value, string $currency)
    {
        ...
        $ret = $symbol . (\mb_strlen($symbol, 'UTF-8') > 2 ? " " : '') . $value;
    ...
    public function setAttribute(int $attr, int $value)
    {
        if (!\in_array($attr, self::SUPPORTED_ATTRIBUTES)) {
            $message = \sprintf('The available attributes are: %s', \implode(', ', \array_keys(self::SUPPORTED_ATTRIBUTES)));
        ...
        if (self::SUPPORTED_ATTRIBUTES['ROUNDING_MODE'] === $attr && $this->isInvalidRoundingMode($value)) {
            $message = \sprintf('The supported values for ROUNDING_MODE are: %s', \implode(', ', \array_keys(self::ROUNDING_MODES)));
            throw new MethodArgumentValueNotImplementedException(__METHOD__, 'attr', $value, $message);
        }
        ...
        $this->initializedAttributes[$attr] = \true;
        return \true;
    }
    /**
    ...
    {
        if (\in_array($value, self::ROUNDING_MODES, \true)) {
            return \false;
        }
        return \true;
    }
