...
use Isolated\Symfony\Component\Security\Core\Authorization\Strategy\AffirmativeStrategy;
use Isolated\Symfony\Component\Security\Core\Authorization\Strategy\ConsensusStrategy;
use Isolated\Symfony\Component\Security\Core\Authorization\Strategy\PriorityStrategy;
use Isolated\Symfony\Component\Security\Core\Authorization\Strategy\UnanimousStrategy;
use Isolated\Symfony\Component\Security\Core\Authorization\Voter\CacheableVoterInterface;
    ...
    private function collectResults(TokenInterface $token, array $attributes, $object) : \Traversable
    {
        foreach ($this->getVoters($attributes, $object) as $voter) {
            ...
            if (!\is_int($result) || !(self::VALID_VOTES[$result] ?? \false)) {
                trigger_deprecation('symfony/security-core', '5.3', 'Returning "%s" in "%s::vote()" is deprecated, return one of "%s" constants: "ACCESS_GRANTED", "ACCESS_DENIED" or "ACCESS_ABSTAIN".', \var_export($result, \true), \get_debug_type($voter), VoterInterface::class);
            }
            (yield $result);
        }
    }
