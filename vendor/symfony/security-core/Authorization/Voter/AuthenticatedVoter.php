...
use Isolated\Symfony\Component\Security\Core\Authentication\Token\SwitchUserToken;
...
class AuthenticatedVoter implements CacheableVoterInterface
{
    ...
    public function vote(TokenInterface $token, $subject, array $attributes)
    {
        ...
        foreach ($attributes as $attribute) {
            if (null === $attribute || self::IS_AUTHENTICATED_FULLY !== $attribute && self::IS_AUTHENTICATED_REMEMBERED !== $attribute && self::IS_AUTHENTICATED_ANONYMOUSLY !== $attribute && self::IS_AUTHENTICATED !== $attribute && self::IS_ANONYMOUS !== $attribute && self::IS_IMPERSONATOR !== $attribute && self::IS_REMEMBERED !== $attribute) {
                continue;
            }
            ...
            if (self::IS_AUTHENTICATED === $attribute && (\method_exists($this->authenticationTrustResolver, 'isAuthenticated') ? $this->authenticationTrustResolver->isAuthenticated($token) : $token && $token->getUser())) {
                return VoterInterface::ACCESS_GRANTED;
            }
    ...
    {
        return \in_array($attribute, [self::IS_AUTHENTICATED_FULLY, self::IS_AUTHENTICATED_REMEMBERED, self::IS_AUTHENTICATED_ANONYMOUSLY, self::IS_AUTHENTICATED, self::IS_ANONYMOUS, self::IS_IMPERSONATOR, self::IS_REMEMBERED, self::PUBLIC_ACCESS], \true);
    }
