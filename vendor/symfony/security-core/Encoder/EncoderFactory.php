...
use Isolated\Symfony\Component\PasswordHasher\Hasher\PasswordHasherAwareInterface;
use Isolated\Symfony\Component\PasswordHasher\Hasher\PasswordHasherFactory;
...
class EncoderFactory implements EncoderFactoryInterface
{
    ...
    private function createEncoder(array $config, bool $isExtra = \false) : PasswordEncoderInterface
    {
        ...
        if ($isExtra || !\in_array($config['class'], [NativePasswordEncoder::class, SodiumPasswordEncoder::class], \true)) {
            return $encoder;
        }
        if ($rawConfig ?? null) {
    ...
    private function getEncoderConfigFromAlgorithm(array $config) : array
    {
        if ('auto' === $config['algorithm']) {
            ...
            foreach ([SodiumPasswordEncoder::isSupported() ? 'sodium' : 'native', 'pbkdf2', $config['hash_algorithm']] as $algo) {
                ...
                $encoderChain[] = $this->createEncoder($config, \true);
            }
            return ['class' => MigratingPasswordEncoder::class, 'arguments' => $encoderChain];
        }
        if ($fromEncoders = $config['migrate_from'] ?? \false) {
            ...
            foreach ($fromEncoders as $name) {
                if ($encoder = $this->encoders[$name] ?? \false) {
                    $encoder = $encoder instanceof PasswordEncoderInterface ? $encoder : $this->createEncoder($encoder, \true);
                } else {
                    $encoder = $this->createEncoder(['algorithm' => $name], \true);
                }
        ...
        switch ($config['algorithm']) {
            case 'plaintext':
                return ['class' => PlaintextPasswordEncoder::class, 'arguments' => [$config['ignore_case']]];
            case 'pbkdf2':
                return ['class' => Pbkdf2PasswordEncoder::class, 'arguments' => [$config['hash_algorithm'] ?? 'sha512', $config['encode_as_base64'] ?? \true, $config['iterations'] ?? 1000, $config['key_length'] ?? 40]];
            case 'bcrypt':
                $config['algorithm'] = 'native';
                $config['native_algorithm'] = \PASSWORD_BCRYPT;
                return $this->getEncoderConfigFromAlgorithm($config);
            case 'native':
                return ['class' => NativePasswordEncoder::class, 'arguments' => [$config['time_cost'] ?? null, ($config['memory_cost'] ?? 0) << 10 ?: null, $config['cost'] ?? null] + (isset($config['native_algorithm']) ? [3 => $config['native_algorithm']] : [])];
            case 'sodium':
                return ['class' => SodiumPasswordEncoder::class, 'arguments' => [$config['time_cost'] ?? null, ($config['memory_cost'] ?? 0) << 10 ?: null]];
            case 'argon2i':
        ...
        return ['class' => MessageDigestPasswordEncoder::class, 'arguments' => [$config['algorithm'], $config['encode_as_base64'] ?? \true, $config['iterations'] ?? 5000]];
    }
}
